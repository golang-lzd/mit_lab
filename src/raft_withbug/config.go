...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			...
			log.Printf("%v: log %v; server %v ApplyMsg:%v \n", i, cfg.logs[i], cfg.logs[j], m)
...
func (cfg *config) checkNoLeader() {
	...
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			if is_leader {
				log.Printf("expected no leader among connected servers, but %v claims to be leader", i)
